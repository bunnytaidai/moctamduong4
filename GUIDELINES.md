# Muxintang Moxibustion Wellness Center - Hướng Dẫn Vận Hành & Phát Triển (v3.9.0-debug-11)
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Trang Chủ | v3.9.0-debug-11 | `./index.html` | Cấu trúc DOM chính hiển thị cuốn sách 3D và thanh liên hệ | (Có sẵn trong source code) |
| 2 | Phong Cách Trang Chủ | v3.9.0-debug-11 | `./index.css` | Thiết kế giao diện, phối cảnh 3D, nền trong suốt, hiệu ứng lật trang tràn khung và bóng đổ tĩnh sâu chân thực (loại bỏ hiệu ứng nổi bóng đổ động khi ấn lật trang) | (Có sẵn trong source code) |
| 3 | Kịch Bản Trang Chủ | v3.9.0-debug-11 | `./main.js` | Logic vận hành Flipbook mượt mà, cố định chế độ trang đôi (orientation: landscape) trên tất cả thiết bị để giữ nguyên vị trí và kích thước gốc khi chạm vuốt | (Có sẵn trong source code) |
...
| 7 | Trình Quản Lý Dữ Liệu | v3.9.0-debug-11 | `./data-manager.js` | Đồng bộ dữ liệu, tự động thêm/xóa/sửa trang thực đơn theo tệp ảnh thực tế trong thư mục images | (Có sẵn trong source code) |
...
| 12 | Trang Quản Trị | v3.9.0-debug-11 | `./admin.html` | Giao diện quản trị, WYSIWYG canvas, kéo thả sắp xếp trang và tuỳ chỉnh chữ nâng cao | (Có sẵn trong source code) |
| 13 | Kịch Bản Quản Trị | v3.9.0-debug-11 | `./admin.js` | Cấu hình đám mây, quét repo và tự động đẩy mã nguồn lên GitHub | (Có sẵn trong source code) |
...
| **Cuốn sách bị nổi lên nhấp nhô khi ấn lật trang (tăng bóng đổ động)** | Cấu hình `.flipbook.flipping-active` trước đó áp dụng `box-shadow` động cực lớn khi lật khiến cuốn sách trông có cảm giác trồi sụt 3D quá mức. | Loại bỏ thuộc tính `box-shadow` trong `.flipbook.flipping-active` ở `index.css`. Bóng đổ sẽ giữ nguyên giá trị tĩnh chân thực của cuốn sách khi lật, giúp sách nằm im vững chắc. |
