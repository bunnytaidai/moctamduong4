# Muxintang Moxibustion Wellness Center - Hướng Dẫn Vận Hành & Phát Triển (v3.9.0-debug-03)
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | Trang Chủ | v3.9.0-debug-03 | `./index.html` | Cấu trúc DOM chính hiển thị cuốn sách 3D và thanh liên hệ | (Có sẵn trong source code) |
| 2 | Phong Cách Trang Chủ | v3.9.0-debug-03 | `./index.css` | Thiết kế giao diện, phối cảnh 3D, nền trong suốt, hiệu ứng lật trang tràn khung và bóng đổ 3D sâu cực mạnh (không phóng to cuốn sách khi lật) | (Có sẵn trong source code) |
| 3 | Kịch Bản Trang Chủ | v3.9.0-debug-03 | `./main.js` | Logic vận hành Flipbook mượt mà, thời gian lật 600ms, khoảng cách vuốt tự nhiên và gáy sách tự động ẩn trên di động dọc | (Có sẵn trong source code) |
...
| 12 | Trang Quản Trị | v3.9.0-debug-03 | `./admin.html` | Giao diện quản trị, WYSIWYG canvas, kéo thả sắp xếp trang và tuỳ chỉnh chữ nâng cao | (Có sẵn trong source code) |
| 13 | Kịch Bản Quản Trị | v3.9.0-debug-03 | `./admin.js` | Logic kéo thả, thiết lập thuộc tính nâng cao của chữ và dịch vụ, đồng bộ GitHub Cloud | (Có sẵn trong source code) |
...
| **Gáy sách đè dọc chia đôi giữa trang bìa (chế độ trang đơn Mobile dọc)** | Khi màn hình hẹp, StPageFlip tự động chuyển sang chế độ trang đơn (Portrait) và căn giữa. Bóng gáy sách overlay nằm ở `left: 50%` sẽ đè thẳng lên tâm trang đơn. | Kiểm tra hướng hiển thị thông qua `pageFlip.getOrientation()`. Nếu là `portrait` (trang đơn), ẩn hoàn toàn `#book-crease` và `#page-stack`. Nếu là `landscape` (trang đôi), hiện lại bình thường. |
